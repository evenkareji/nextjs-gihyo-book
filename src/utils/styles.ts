...
*/
type SpaceThemeKeys = keyof typeof theme.space //a|b
...
export function toPropValue<T>(
  ...
  if (isResponsivePropType(prop)) {
    ...
    //配列にjoinメソッドをつけて配列の要素を連結したり、区切ったりする
    return result.join('\n')
  }
...
const LINE_HEIGHT_KEYS = new Set(['line_height'])
/**
...
function isSpaceThemeKeys(prop: any, theme: AppTheme): prop is SpaceThemeKeys {
  // key==propのもので文字数が0より大きいプロパティを配列で取り出す
